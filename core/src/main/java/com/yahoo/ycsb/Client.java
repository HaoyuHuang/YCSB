...
class StatusThread extends Thread {
	 ...
	 *            The number of seconds between status updates.
	 * @param trackJVMStats
	 *            Whether or not to track JVM stats.
	 */
	public StatusThread(CountDownLatch completeLatch, List<ClientThread> clients, String label, boolean standardstatus,
		...
		try {
			if (dotransactions) {
				...
				while (((opcount == 0) || (opsdone < opcount)) && !workload.isStopRequested()) {
					...
					Status status = workload.doTransaction(db, workloadstate);
	...
	private static Properties parseArguments(String[] args) {
		...
		if (argindex != args.length) {
			...
			if (argindex < args.length) {
				...
				System.out.println(
						"An argument specifier without corresponding value was found at the end of the supplied "
